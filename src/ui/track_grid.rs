...
pub const DEFAULT_TRACK_SIZE: egui::Vec2 = egui::vec2(320.0 / 1.4, 180.0 / 1.4);
...
pub struct TrackGrid {
    ...
    prev_state: State,
    curr_state: State,
    track_index: u32, // Kinda "cursor"

    // Options
    min_track_size: egui::Vec2,
    max_columns: u32,
    spacing: f32,
}
...
impl TrackGrid {
    pub fn new(id_source: impl std::hash::Hash + std::fmt::Debug) -> Self {
        Self {
            ...
            track_index: 0,
            min_track_size: DEFAULT_TRACK_SIZE,
    ...
    pub fn show<R>(
        mut self,
        ui: &mut egui::Ui,
        grid: impl FnOnce(&mut TrackGridContext) -> R,
        ...
        ui.ctx()
            .check_for_id_clash(self.id, self.available_rect, "TrackGrid");
    ...
    fn next_frame_rect(&mut self) -> egui::Rect {
        ...
        assert!(self.spacing <= self.min_track_size.x);

        // increment the amount of tracks for the next frame
        self.curr_state.num_tracks += 1;

        let num_tracks = self.prev_state.num_tracks;
        if num_tracks == 0 {
            return egui::Rect::NOTHING;
        }

        let max_columns = self.max_columns;
        let minimum_size = self.min_track_size;
        ...
        let x_index = self.track_index % total_columns;
        let y_index = self.track_index / total_columns;

        let x = {
            let mut x = x_index as f32 * (w + self.spacing);

            // vertically center the last row
            let total_rows = num_tracks / total_columns + 1;
            if (y_index + 1) == total_rows {
                let nb_items = num_tracks - (total_rows - 1) * total_columns; // nb. of items on the last row
...
#[allow(dead_code)]
impl TrackGrid {
    pub fn min_track_size(mut self, min_track_size: egui::Vec2) -> Self {
...
pub struct TrackGridContext<'a> {
    layout: &'a mut TrackGrid,
    ui: &'a mut egui::Ui,
}

impl<'a> TrackGridContext<'a> {
    pub fn track_frame(&mut self, add_contents: impl FnOnce(&mut egui::Ui)) -> egui::Response {
